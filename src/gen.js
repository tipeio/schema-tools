...
const makeModelActions = (modelTypes, schemaTemplateData, userSchema) => {
  return modelTypes.reduce(
    (fields, type) => {
      ...
      fields.queries[names.cap] = queryResolvers.getOne(
        type,
        schemaTemplateData,
        userSchema,
        names
      )
      fields.queries[names.many] = queryResolvers.getMany(
        type,
        schemaTemplateData,
        userSchema,
        names
      ...
      fields.mutations[names.create] = mutationResolvers.create(
        type,
        schemaTemplateData,
        userSchema
      )
      fields.mutations[names.remove] = mutationResolvers.remove(
        type,
        schemaTemplateData,
        userSchema
      )
      fields.mutations[names.update] = mutationResolvers.update(
        type,
        schemaTemplateData,
        userSchema
      )
...
const makePageActions = (pageTypes, schemaTemplateData, userSchema) => {
  return pageTypes.reduce(
    (fields, type) => {
      fields.queries[type.name] = queryResolvers.getPage(
        type,
        schemaTemplateData,
        userSchema
      )
...
export const createSchema = () => {
  ...
  const types = schemaTemplateData.types
    .filter(type => !ourTypes[type.name])
    .map(addDefaults)

  const modelTypes = types.filter(
    type => type.usesDirectives && type.directives.type
  )
  const pageTypes = types.filter(
    type => type.usesDirectives && type.directives.page
  )
